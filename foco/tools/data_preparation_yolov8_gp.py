...
import shutil
import re
from misc_tools import dict_to_namespace, get_files_in_directory, resize_and_copy_image, copy_file, save_to_txt_file, read_txt_file, save_to_csv_file, delete_file

config_file = '../configs/data_preparation_yolov8_gp_config.yaml'

#根据原始数据内容及YOLOv8 GP标注要求，转换标注数据，并保存
def convert_annotation_YOLOv8_GP_3DBox(source_dir, target_dir, file_name, image_size, resize_value):
    #检查源目录是否存在，如果没有则报错返回
    ...
    #截取“***.jpg”中的***
    rematch = re.match(r'^(.*)\.jpg$', file_name).group(1)
    ...
    for line in content_lines:
        ...
        for i in range(17):       #只取前八个（x,y）点
            if i == 0:      #类型：default --> 0:car; default_bus --> 1:bus
                single_line_converted.append(single_line[i])
                '''
                if(single_line[i] == "0"):
                ...
                single_line_converted.append(str(float(single_line[i])/image_size[1]))
                single_label_converted.append(float(single_line[i])/image_size[1]) 
...
def main():
    ...
    while(image_to_label_flag !=2 ):
        ...
        for i in range(total_data_num_image):
            if(list_file_image[i] != '.DS_Store'):
                prefix_image = re.match(r'^(.*)\.jpg$', list_file_image[i]).group(1)
    ...
    while(label_to_image_flag !=2 ):
        ...
        for i in range(total_data_num_label):
            if(list_file_label[i] != '.DS_Store'):
                ...
                if (prefix_label + '.jpg') not in list_file_image:
    ...
    print("\033[1;34mStep1Clear:image和label一一对应关系检查完成，符合要求\033[0m")
        ...
        for i in range(total_data_num_label):
            if(list_file_label[i] != '.DS_Store'):  # in case of Mac
                if (read_txt_file(config.original_data_path.label + list_file_label[i]) == []):
                    ...
                    delete_file(config.original_data_path.image + list_file_label[i].replace(".txt",".jpg"))   #删除对应image
                    exist_flag = 1
                    empty_label_flag = 1
        if(exist_flag == 0):
            empty_label_flag = 2
                ...
                for line in content_lines:
                    ...
                    for j in range(len(single_line)):
                        if j == 0:
                            ...
                            #检查数据归一情况
                            if(float(single_line[j])/config.image_size_MAXX > 1 or float(single_line[j]) < 0):
                                print(f"发现大于图片X-MAX的数/小于零的数，在 {list_file_label[i]}")
                            ...
                            #检查数据归一情况
                            if(float(single_line[j])/config.image_size_MAXY > 1 or float(single_line[j]) < 0):
                                print(f"发现大于图片Y-MAX的数/小于零的数，在 {list_file_label[i]}")
                                should_keep_line = False  # 该行不符合条件，标记为删除
                                exist_flag = 1
                                exceed_label_flag = 1
                                break  # 不再继续检查当前行
    ...
    print("\033[1;34mStep2Clear:所有label数据检查合法\033[0m")
    
    print("\033[1;34mStep3: 再次检查image和label，确保一一匹配，不匹配的删除；空值label删除\033[0m")
    print("->>>核对项1：有image的都必须有label")
    image_to_label_flag = 0     # 0:初始态；1：存在有图片没label；2：不存在有图片没label
    while(image_to_label_flag !=2 ):
        exist_flag = 0  #异常存在flag
    ...
    print("->>>核对项3：检查空label，空label删除label及对应image")
    empty_label_flag = 0     # 0:初始态；1：存在空label；2：不存在空label
    while(empty_label_flag != 2):
        exist_flag = 0
        try:
        ...
        if(exist_flag == 0):
            empty_label_flag = 2
    print("-<<<核对项3完成:所有空label及对应image都已删除")
    print("\033[1;34mStep3Clear:image和label一一对应关系、空label检查完成，符合要求\033[0m")

    #从整理好的原始数据路径读取数据，并按照train, val, test分割数据
    print("\033[1;34mStep4: 从整理好的原始数据路径读取数据，并按照train, val, test分割数据\033[0m")
    #读取数据集，并按照比例制造随机数
    try:
        total_data_num_image, list_file_image = get_files_in_directory(config.original_data_path.image)
        total_data_num_label, list_file_label = get_files_in_directory(config.original_data_path.label)
        ...
        if i in trainval_list:
            if i in train_list:     #train数据集
                if (list_file_image[i] != '.DS_Store'): # in case of Mac
                    ...
                    resize_and_copy_image(config.original_data_path.image, config.destination_data_path.train.image, list_file_image[i], (config.resize_MAXX, config.resize_MAXY))
                    train_label = convert_annotation_YOLOv8_GP_3DBox(config.original_data_path.label, config.destination_data_path.train.label, list_file_image[i], [config.image_size_MAXX, config.image_size_MAXY], [config.resize_MAXX, config.resize_MAXY])   #将原始label转换成FocoMono3D label并保存到train/label下
                ...
                if (list_file_image[i] != '.DS_Store'): # in case of Mac
                    ...
                    resize_and_copy_image(config.original_data_path.image, config.destination_data_path.val.image, list_file_image[i], (config.resize_MAXX, config.resize_MAXY))
                    convert_annotation_YOLOv8_GP_3DBox(config.original_data_path.label, config.destination_data_path.val.label, list_file_image[i], [config.image_size_MAXX, config.image_size_MAXY], [config.resize_MAXX, config.resize_MAXY])            #将原始label转换成FocoMono3D label并保存到train/label下
        else:       #test数据集
            if (list_file_image[i] != '.DS_Store'): # in case of Mac    
                #copy_file(config.original_data_path.image, config.destination_data_path.test.image, list_file_image[i])         #将原始图片拷贝到testimage下
                resize_and_copy_image(config.original_data_path.image, config.destination_data_path.test.image, list_file_image[i], (config.resize_MAXX, config.resize_MAXY))
    ...
    print("\033[1;34mStep4Clear:train, val, test数据分割完成\033[0m")

    print("\033[1;32mAllStepsClear:数据集合准备完成\033[0m")
