...
def main():
    ...
    print("Step1: 检查image和label，确保一一匹配，不匹配的删除")
    ...
    print("----StepClear:image和label一一对应关系检查完成，符合要求")

    #检查label数据合法性
    print("Step2: 检查label数据合法性")
    ...
    while(empty_label_flag != 2):
        ...
        for i in range(total_data_num_label):
            if (read_txt_file(config.original_data_path.label + list_file_label[i]) == []):
                print(f"发现空label: {list_file_label[i]} ")
    ...
    print("->>>核对项2：检查label值超出图片最大尺寸，超出尺寸，删除label及对应image")
    ...
    while(exceed_label_flag != 2):
        ...
        if(exist_flag == 0):
            exceed_label_flag = 2
    print("-<<<核对项2完成：超出尺寸，删除label及对应image")
    print("----StepClear:所有label数据检查合法")
    
    #从整理好的原始数据路径读取数据，并按照train, val, test分割数据
    print("Step3: 从整理好的原始数据路径读取数据，并按照train, val, test分割数据")
    #读取数据集，并按照比例制造随机数
    try:
        total_data_num_image, list_file_image = get_files_in_directory(config.original_data_path.image)
        total_data_num_label, list_file_label = get_files_in_directory(config.original_data_path.label)
    ...
    save_to_txt_file(config.image_paths_dir, config.train_image_paths_name, train_image_paths)
    save_to_txt_file(config.labels_dir, config.train_lables_name, train_labels)    
